...
const dom = {
  ...
  ideSelector: document.getElementById('ideSelector'),
  ideOptCodex: document.getElementById('ideOptCodex'),
  ideOptAntigravity: document.getElementById('ideOptAntigravity'),
...
function connectSocket() {
  ...
  socket.addEventListener('message', (event) => {
    ...
    if (message.type === 'snapshot') {
      ...
      if (message.data?.ide) {
        state.currentIde = message.data.ide;
        state.ideName = message.data.ideName || message.data.ide;
        updateIdeSelector();
      }
      renderDashboard();
    }
...
function updateIdeSelector() {
  const buttons = dom.ideSelector?.querySelectorAll('.ide-option') || [];
  for (const btn of buttons) {
    btn.classList.toggle('is-active', btn.dataset.ide === state.currentIde);
  }
  updateBranding();
}

function updateBranding() {
  const ide = state.currentIde;
  const name = state.ideName;
  if (dom.brandEyebrow) dom.brandEyebrow.textContent = `Realtime ${name} office`;
  if (dom.brandTitle) dom.brandTitle.textContent = `${name.split(' ')[0]} Pixel Lab`;
  document.title = `${name.split(' ')[0]} Pixel Lab`;
}
...
async function switchIde(ideId) {
  ...
  try {
    ...
    state.snapshot = data;
    state.currentIde = data.ide || ideId;
    state.ideName = data.ideName || ideId;
    updateIdeSelector();
    renderDashboard();
  } catch {
    dom.connectionPill.textContent = 'IDE switch failed';
  }
}
